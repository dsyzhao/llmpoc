...
def process_response(query, session_state, trial_id, query_id, resp):
    ...
    for event in event_stream: 
        # print (f"\n{event = }\n")
        try:
            pprint (event['trace']['trace']['orchestrationTrace']['invocationInput']['actionGroupInvocationInput'])
        except:
            pass
    ...
    for i in range(number_trials):
        ...
        for query in queries_list:
            ...
            if type(query) == dict:
                ...
                if "query" in query:
                    query = query["query"]
            print(f"Query: {query}, Invocation Id: {invocation_id}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_file', type=str, required=True, default='test.json') # json input file containing test data
    parser.add_argument('--agent_id', type=str, required=True, default='HYCYYD7WKC') #ID for the agent
    parser.add_argument('--agent_alias_id', type=str, default="7OAV2UGXZ9") #ID for agent alias
    parser.add_argument('--region', type=str, default="us-east-1") #region
    parser.add_argument('--number_trials', type=int, default=1) # number of trials per query
